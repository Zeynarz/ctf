...
char *temp_buffer;
unsigned long magic; // random bytes, acts as a canary prob
...
/*
 Accept characters only in [A-Z|a-z] and translate to [A-Z], make it all upper case
...
static noinline long pm_sanitize_request(void)
{
    ...
    request_t *req = pm_queue_peek(&incoming_queue); // this returns the kernel ptr, not obj
    if(!req) goto end;
    ...
    for(int i = 0; i < STRING_SZ; i++)
    {
        ...
        if(req->str[i] > 0x60 && req->str[i] < 0x7b) // char is in a-z
            temp_buffer[ptr++] = req->str[i]-0x20;

        else if(req->str[i] > 0x40 && req->str[i] < 0x5b) // char is in A-Z
    ...
    // mov the request from the start of the queue to the end fo the queue
...
static noinline long pm_process_request(void)
{
    ...
    if(!req) goto end;
    if(req->magic != magic) goto end; // sanitization check
    ...
    if(req->type == RAW)
    {
        ...
        {
            // sanitize, then cut the string in half and copy over to temp_buffer
...
{   
    ...
    switch(cmd)
    {
        case QUEUE:
            // whats arg_t size??
            // are we supposed to pass in a pointer to an arg object in userspace?
        ...
        case SANITIZE:
            ...
            sanitized = true; // sanitize can only be called once
