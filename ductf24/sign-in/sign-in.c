...
void sign_up() {
    ...
    user->uid = UID++;
    printf("username: ");
    read(0, user->username, 8); // no append null byte right? so no one byte overflow/off-by-one
    printf("password: ");
    read(0, user->password, 8);
    entry->user = user;

    // insert user_entry into linked list
    // root is always the head node
