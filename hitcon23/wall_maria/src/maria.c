...
static uint64_t maria_mmio_read(void *opaque, hwaddr addr, unsigned size) {
    ...
    uint64_t val = 0;
    switch (addr) {
        // addr = addr in devmem - 0xfebd0000?
        // so address is not really an addr, but an option
        // so you can read and set the state.src, state.off
        //
        // Im pretty sure that the maria object is in the host/qemu process memory
        // state.src = an address in the guest physical memory
        // and the buffer is in the maria object
        // so you can read stuff from the maria object into the state.src
        case 0x00:
            // write memory in buffer into the guest physical memory
...
static void maria_mmio_write(void *opaque, hwaddr addr, uint64_t val, unsigned size) {
    // it only writes if size 32 is used? its divided by 8?
    MariaState *maria = (MariaState *)opaque;
    switch (addr) {
        case 0x00:
            // write memory from guest physical memory into buffer
