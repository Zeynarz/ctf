...
static void proc_flask(int id) {
    int in_fd = to_flask[id][0]; // read from main
    int out_fd = to_main[id][1]; // write to main

    char **argv = calloc(MAX_ARGC, sizeof(*argv)); // MAX_ARGC = 16
    struct request *req = malloc(MAX_REQ); // MAX_REQ = 259 (since max ingredient len is 256)
...
void init_flasks(int n) {
    ...
    for (int i = 0; i < n; i++) {
        pipe2(to_flask[i], O_DIRECT); // packet mode
    ...
    for (int i = 0; i < n; i++) {
        flasks[i] = fork();
        if (flasks[i] == 0) // To check if parent process or child process
            proc_flask(i);
    }
